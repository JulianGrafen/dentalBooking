-- Waitlist signup: match blocking appointments by start time only and persist their slot times.
-- The UI offers waitlist for any treatment at an occupied start time; end times may differ.
...
begin
  ...
  if requested_start_time is null then
  ...
  into blocking_start, blocking_end
  ...
  if blocking_start is null then
    raise exception 'appointment slot is available';
  end if;

  slot_minutes := extract(epoch from (blocking_end - blocking_start)) / 60;

  if slot_minutes < 15 or slot_minutes > 180 then
    raise exception 'invalid appointment duration';
  end if;

  local_start := blocking_start at time zone 'Europe/Berlin';
  local_end := blocking_end at time zone 'Europe/Berlin';
